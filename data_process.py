...
class NewsDeduplicator:
    ...
    def jaccard_similarity_minhash(self, sig1: List[int],
                                   sig2: List[int]) -> float:
    ...
    def is_duplicate(self, item1: Dict, item2: Dict) -> bool:
        ...
        # 应用三重阈值
        return (title_sim > self.title_threshold
                and content_sim > self.content_threshold
                and semantic_dist <= self.simhash_threshold)

    def load_and_preprocess_data(
            self, csv_file_path: str = "/mnt/data/Finance/risk_nasdaq/2.csv"):
        """从本地CSV文件加载并预处理数据"""
        print("正在加载CSV文件...")
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                article_content = str(row.get('Article', '')) or str(
                    row.get('Textrank_summary', ''))

                # 提取标题
                title = str(row.get('Article_title', '')) or str(
                    row.get('Stock_symbol', ''))

                # 提取标签（风险评分）
                risk_score = row.get('risk_deepseek', '')
                labels = f"risk_score:{risk_score}" if pd.notna(
                    risk_score) else ""

                # 提取股票代码
                stock_symbol = str(row.get('Stock_symbol', '')) if pd.notna(
                    row.get('Stock_symbol')) else ""

                # 提取日期
                date = str(row.get('Date', '')) if pd.notna(
                    row.get('Date')) else ""
    ...
    def process_dataset(
            self,
            csv_file_path: str = "/mnt/data/Finance/risk_nasdaq/2.csv",
...
if __name__ == "__main__":
    ...
    try:
        unique_news = deduplicator.process_dataset(
            csv_file_path=
            "/mnt/data/Finance/risk_nasdaq/risk_deepseek_cleaned_nasdaq_news_full.csv",  #  替换为你的路径
