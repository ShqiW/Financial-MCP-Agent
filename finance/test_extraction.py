...
def extract_stock_info(query):
    ...
    if company_name:
        # 移除常见的无意义词汇
        stop_words = [
            '的', '这个', '这只', '一下', '看看', '了解', '分析', '帮我', '我想', '给我', '财务状况',
            '投资价值', '基本面情况', '这只股票', '这个股票'
        ]
...
def test_extraction():
    ...
    for i, (query, expected_company,
            expected_stock) in enumerate(test_cases, 1):
        ...
        print(f"      查询: {query}")
        print(
            f"      期望: 公司={expected_company or 'None'}, 代码={expected_stock or 'None'}"
        )
        print(
            f"      实际: 公司={company_name or 'None'}, 代码={stock_code or 'None'}"
        )

        if not test_passed:
            if not company_match:
                print(
                    f"      ❌ 公司名称不匹配: 期望 '{expected_company}', 实际 '{company_name}'"
                )
            if not stock_match:
                print(
                    f"      ❌ 股票代码不匹配: 期望 '{expected_stock}', 实际 '{stock_code}'"
                )
