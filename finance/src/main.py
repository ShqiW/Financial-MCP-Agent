...
# 日志和状态管理相关导入
from finance.src.utils.logging_config import setup_logger, SUCCESS_ICON, ERROR_ICON, WAIT_ICON
from finance.src.utils.state_definition import AgentState
from finance.src.utils.execution_logger import initialize_execution_logger, finalize_execution_logger, get_execution_logger

# 智能体模块导入 - 五个核心分析智能体
from finance.src.agents.summary_agent import summary_agent  # 总结智能体：整合所有分析结果
from finance.src.agents.value_agent import value_agent  # 估值智能体：分析股票估值水平
from finance.src.agents.technical_agent import technical_agent  # 技术分析智能体：分析价格趋势和技术指标
from finance.src.agents.fundamental_agent import fundamental_agent  # 基本面智能体：分析财务状况和盈利能力
from finance.src.agents.news_agent import news_agent  # 新闻分析智能体：分析新闻情感和风险
...
async def main():
    ...
    try:
        ...
        # 提取并打印最终报告
        if final_state and final_state.get(
                "data") and "final_report" in final_state["data"]:
