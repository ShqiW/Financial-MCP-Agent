...
from finance.src.utils.state_definition import AgentState
from finance.src.tools.mcp_client import get_mcp_tools
from finance.src.utils.logging_config import setup_logger, ERROR_ICON, SUCCESS_ICON, WAIT_ICON
from finance.src.utils.execution_logger import get_execution_logger
...
async def technical_agent(state: AgentState) -> AgentState:
    ...
    """
    logger.info(
        f"{WAIT_ICON} TechnicalAgent: Starting technical analysis using ReAct framework."
    )
    ...
    if not user_query:
        logger.error(
            f"{ERROR_ICON} TechnicalAgent: User query is missing in state data."
        )
        current_data["technical_analysis_error"] = "User query is missing."
        execution_logger.log_agent_complete(agent_name, current_data, 0, False,
    ...
    try:
        ...
        if not all([api_key, base_url, model_name]):
            logger.error(
                f"{ERROR_ICON} TechnicalAgent: Missing OpenAI environment variables."
            )
            current_data[
                "technical_analysis_error"] = "Missing OpenAI environment variables."
            execution_logger.log_agent_complete(
                agent_name, current_data,
                time.time() - agent_start_time, False,
        ...
        logger.info(
            f"{WAIT_ICON} TechnicalAgent: Creating ChatOpenAI with model {model_name}"
        )
        # 创建LLM实例，设置合适的参数
        llm = ChatOpenAI(
            model=model_name,
        ...
        try:
            ...
            if not mcp_tools:
                logger.error(
                    f"{ERROR_ICON} TechnicalAgent: No MCP tools available.")
                current_data[
                    "technical_analysis_error"] = "No MCP tools available."
                execution_logger.log_agent_complete(
                    agent_name, current_data,
                    time.time() - agent_start_time, False,
            ...
            logger.info(
                f"{SUCCESS_ICON} TechnicalAgent: Successfully loaded {len(mcp_tools)} tools."
            )
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。"""
            ...
            if "messages" in response and isinstance(response["messages"],
                                                     list):
            ...
            logger.info(
                f"{SUCCESS_ICON} TechnicalAgent: Successfully completed technical analysis."
            )
            ...
            current_metadata[
                "technical_agent_execution_time"] = f"{execution_time:.2f} seconds"
            ...
            execution_logger.log_agent_complete(
                agent_name, {
                    "technical_analysis_length":
                    len(final_output),
                    "analysis_preview":
                    final_output[:500]
                    if len(final_output) > 500 else final_output,
                    "llm_execution_time":
                    execution_time,
            ...
            logger.error(
                f"{ERROR_ICON} TechnicalAgent: Error in MCP or agent execution: {e}",
                exc_info=True)
            current_data[
                "technical_analysis_error"] = f"Error in MCP or agent execution: {e}"
        ...
        logger.error(
            f"{ERROR_ICON} TechnicalAgent: Error during execution: {e}",
            exc_info=True)
        current_data[
            "technical_analysis_error"] = f"Error during execution: {e}"
        current_metadata["technical_agent_error"] = str(e)
        execution_logger.log_agent_complete(agent_name, current_data,
                                            time.time() - agent_start_time,
                                            False, str(e))
...
async def test_technical_agent():
    ...
    current_weekday_cn = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六",
                          "星期日"][current_datetime.weekday()]
    ...
    print(
        result.get("data", {}).get("technical_analysis", "No analysis found"))
