...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not all([api_key, base_url, model_name]):
            ...
            current_data[
                "fundamental_analysis_error"] = "Missing OpenAI environment variables."
        ...
        try:
            ...
            if not mcp_tools:
                ...
                current_data[
                    "fundamental_analysis_error"] = "No MCP tools available."
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的时间周期或其他工具组合，基于可用信息提供尽可能全面的分析。"""
            ...
            current_metadata[
                "fundamental_agent_execution_time"] = f"{execution_time:.2f} seconds"
            ...
            logger.error(
                f"{ERROR_ICON} FundamentalAgent: Error in MCP or agent execution: {e}",
                exc_info=True)
            current_data[
        ...
        logger.error(
            f"{ERROR_ICON} FundamentalAgent: Error during execution: {e}",
            exc_info=True)
        current_data[
            "fundamental_analysis_error"] = f"Error during execution: {e}"
        current_metadata["fundamental_agent_error"] = str(e)
...
async def test_fundamental_agent():
    ...
    print(result)
    print(
        result.get("data", {}).get("fundamental_analysis",
                                   "No analysis found"))
