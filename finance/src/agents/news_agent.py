...
async def news_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not all([api_key, base_url, model_name]):
            logger.error(
                f"{ERROR_ICON} NewsAgent: Missing OpenAI environment variables."
            )
            current_data[
                "news_analysis_error"] = "Missing OpenAI environment variables."
        ...
        logger.info(
            f"{WAIT_ICON} NewsAgent: Creating ChatOpenAI with model {model_name}"
        )
        # 创建LLM实例，设置合适的参数
        llm = ChatOpenAI(
            model=model_name,
...
请使用可用的工具获取实际新闻数据进行分析，确保情感分析和风险评估的准确性。如果某些新闻无法获取，请基于可用信息提供尽可能全面的分析。"""
            ...
            logger.info(f"{WAIT_ICON} NewsAgent: Calling ReAct agent...")
            ...
            current_metadata[
                "news_agent_execution_time"] = f"{execution_time:.2f} seconds"
            ...
            logger.error(
                f"{ERROR_ICON} NewsAgent: Error in MCP or agent execution: {e}",
                exc_info=True)
            current_data[
        ...
        logger.error(f"{ERROR_ICON} NewsAgent: Error during execution: {e}",
                     exc_info=True)
