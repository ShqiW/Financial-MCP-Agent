...
def truncate_report_at_baseline_time(report_content: str,
                                     current_time_info: str) -> str:
    """
    ...
    try:
        ...
        if prompt in generated_text:
            ...
            if output_length > input_length:
                ...
                report = tokenizer.decode(new_tokens,
...
async def summary_agent(state: AgentState) -> Dict[str, Any]:
    ...
    # 获取之前 Agent的分析结果
    fundamental_analysis = current_data.get("fundamental_analysis",
                                            "Not available")
    technical_analysis = current_data.get("technical_analysis",
                                          "Not available")
    ...
    try:
        ...
        logger.info(
            f"{WAIT_ICON} SummaryAgent: Using model choice: {model_choice}")
        ...
        **重要提醒：**
        ...
        if model_choice == "local":
            # 使用本地FinR1模型
            logger.info(
                f"{WAIT_ICON} SummaryAgent: Using local FinR1 model...")
            ...
            # 使用FinR1模型生成最终报告
            final_report = generate_report_with_finr1(model, tokenizer,
                                                      full_prompt)
            ...
            if not all([api_key, base_url, model_name]):
                ...
                current_data[
                    "summary_error"] = "Missing OpenAI environment variables."
            ...
            # 使用ChatOpenAI模型
            logger.info(
                f"{WAIT_ICON} SummaryAgent: Creating ChatOpenAI with model {model_name}"
            )
            llm = ChatOpenAI(
                model=model_name,
                api_key=api_key,
        ...
        final_report = final_report.replace("```markdown",
                                            "").replace("```", "").strip()

        # 使用正则表达式截断"分析基准时间"那一行之后的内容
        final_report = truncate_report_at_baseline_time(
            final_report, current_time_info)
        ...
        if stock_code == "Unknown Stock" or stock_code == "Extracted from analysis":
            ...
            if safe_company_name == "Unknown_Company" or safe_company_name == "Extracted_from_analysis":
                safe_company_name = user_query.replace(" ", "_").replace(
                    "分析", "").strip()
        ...
        logger.error(
            f"{ERROR_ICON} SummaryAgent: Error generating final report: {e}",
            exc_info=True)
...
async def test_summary_agent():
    ...
    test_state = AgentState(
        ...
        data={
            ...
            "嘉友国际",
            "fundamental_analysis":
            "嘉友国际基本面分析：公司主营业务为跨境物流、供应链贸易以及供应链增值服务。财务状况良好，负债率较低，现金流充裕。近年来业绩稳步增长，毛利率保持在行业较高水平。",
            "technical_analysis":
            "嘉友国际技术分析：短期内股价处于上升通道，突破了200日均线。RSI指标显示股票尚未达到超买区域。MACD指标呈现多头形态，成交量有所放大，支持价格继续上行。",
            "value_analysis":
            "嘉友国际估值分析：当前市盈率为15倍，低于行业平均水平。市净率为1.8倍，处于合理区间。与同行业公司相比，嘉友国际的估值较为合理，具有一定的投资价值。",
            "news_analysis":
            "嘉友国际新闻分析：近期公司发布了2023年业绩预告，预计净利润同比增长15-25%，超出市场预期。同时，公司宣布与多家国际物流巨头达成战略合作，市场反应积极。分析师普遍上调了目标价，市场情绪偏向乐观。"
        },
        metadata={})
