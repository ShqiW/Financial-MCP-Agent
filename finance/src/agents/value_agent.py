...
async def value_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if not all([api_key, base_url, model_name]):
            ...
            current_data[
                "value_analysis_error"] = "Missing OpenAI environment variables."
        ...
        try:
            ...
            if not mcp_tools:
                ...
                current_data[
                    "value_analysis_error"] = "No MCP tools available."
...
请使用可用的工具获取实际数据进行分析，而不是基于假设。如果某些数据无法获取，请尝试使用不同的工具或参数组合，基于可用信息提供尽可能全面的分析。请保持回答简洁，避免冗长的描述性文字。所有分析项完成后请直接输出最终结果，不要重复调用工具或进入循环。"""
            ...
            current_metadata[
                "value_agent_execution_time"] = f"{execution_time:.2f} seconds"
            ...
            logger.error(
                f"{ERROR_ICON} ValueAgent: Error in MCP or agent execution: {e}",
                exc_info=True)
            current_data[
                "value_analysis_error"] = f"Error in MCP or agent execution: {e}"
        ...
        logger.error(f"{ERROR_ICON} ValueAgent: Error during execution: {e}",
                     exc_info=True)
